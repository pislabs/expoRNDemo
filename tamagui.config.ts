...
import { config as tamaguiConfigV3 } from "@tamagui/config/v3";
...
const size = {
  ...tamaguiConfigV3.tokens.size,
  small: 10,
};

export const tokens = createTokens({
  ...tamaguiConfigV3.tokens,
  size,
  space: { ...size, "-1": -5, "-2": -10 },
  radius: { 0: 0, 1: 3 },
...
const tamaguiConfig = createTamagui({
  ...
  themes: {
    ...tamaguiConfigV3?.themes,
    light: {
      ...tamaguiConfigV3?.themes?.light,
      bg: "#f2f2f2",
      color: tokens.color.black,
    },
    dark: {
      ...tamaguiConfigV3?.themes?.dark,
      bg: "#111",
